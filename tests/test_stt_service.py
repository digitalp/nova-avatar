...
# ── _decode_wav (was _wav_to_f32) ─────────────────────────────────────────────

def test_decode_wav_returns_float_array():
    import numpy as np
    wav = _make_wav(n_samples=160)
    arr, rate = _decode_wav(wav)
    assert arr is not None
    assert arr.dtype == np.float32
    assert len(arr) == 160


def test_decode_wav_normalised():
    import numpy as np
    wav = _make_wav(160)
    arr, rate = _decode_wav(wav)
...
def test_decode_wav_returns_none_on_bad_data():
    arr, rate = _decode_wav(b"not a wav file")
    assert arr is None


# ── _decode_pcm (was _pcm16_to_f32) ──────────────────────────────────────────

def test_decode_pcm_returns_float_array():
    import numpy as np
    pcm = _make_pcm16(100)
    arr = _decode_pcm(pcm, 16000)
    assert arr is not None
    assert arr.dtype == np.float32
    assert len(arr) == 100
